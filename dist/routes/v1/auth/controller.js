...
const login = async (req, res) => {
    ...
    if (!ok)
        return res.status(401).json({ error: "Invalid credentials" });
    const accessToken = (0, jwt_1.signAccessToken)({ sub: user.id, role: user.role, email: user.email }, ACCESS_SECRET, ACCESS_EXPIRES);
    const refreshToken = (0, jwt_1.signRefreshToken)({ sub: user.id }, REFRESH_SECRET, `${REFRESH_DAYS}d`);
...
const refresh = async (req, res) => {
    ...
    try {
        ...
        const newRefresh = (0, jwt_1.signRefreshToken)({ sub: user.id }, REFRESH_SECRET, `${REFRESH_DAYS}d`);
        await (0, auth_1.createRefreshToken)(user.id, newRefresh, (0, auth_1.makeExpiryDate)(REFRESH_DAYS));
